...
def sliding_window_align(ref, query, window_size=50, step=25):
    """滑动窗口比对，用于填补空隙"""
    alignments = []
    ...
    for q_start in range(0, len(query) - window_size + 1, step):
        ...
        # 在参考序列中搜索最佳匹配
        for r_start in range(0, len(ref) - len(query_window) + 1, step):
...
def align_sequences(ref, query):
    ...
    # 合适尺寸的滑动窗口比对
    window_alignments = sliding_window_align(ref, query, window_size=50, step=50)
